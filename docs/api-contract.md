...
- **دسترسی با توکن مشترک:** همهٔ مسیرهای `/api` به‌جز `/api/health` هدر
  `X-Notebook-Token` را می‌خواهند. نبود/نادرستی توکن → `401` با
  `{ "message": "The access token is missing or wrong", "errors": [] }`.
  در حالت production اگر توکن روی سرور تنظیم نشده باشد، همهٔ درخواست‌ها `500` می‌گیرند
  (طراحی عمدی: باز شدن تصادفی API بدتر از از کار افتادن آشکار است).
- `/api/health` باز است، ولی در production نسخهٔ Laravel و PHP را برنمی‌گرداند.
- **تکمبدأ:** مسیرهای غیر از `/api` و `/up` با پوستهٔ SPA (`public/index.html`) پاسخ می‌گیرند،
  پس مسیرهایی مثل `/fa/notes/<uuid>` روی دامنه مستقیم قابل باز شدن‌اند.
- احراز هویت کاربری نداریم (اپ تک‌نفرهٔ محلی)؛ هیچ گارد session/JWT روی این مسیرها نیست.
