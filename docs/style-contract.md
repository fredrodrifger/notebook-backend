...
۱. **تک‌مستأجری:** `stancl/tenancy`، `InitializeTenantMiddleware`، کانکشن `tenant`، مسیر
   ...
   پیاده **نمی‌شوند** — چون این اپ یک دیتابیس و یک کاربر دارد. در عوض
   `ConnectionConstants::APP_CONNECTION` روی تنها کانکشن این اپ (`sqlite`) تنظیم شده تا
   مدل‌ها اتصال خود را صریح اعلام کنند (قاعدهٔ پک).
...
## ۵. تله‌هایی که در عمل خوردیم (و راه‌حل)

- **`CREATED_AT` / `UPDATED_AT` را در اینترفیس اعلام نکن.** `Illuminate\Database\Eloquent\Model`
  همین ثابت‌ها را دارد و کلاس مدل با خطای
  `inherits both Model::CREATED_AT and …Interface::CREATED_AT, which is ambiguous` بستهٔ می‌شود.
  در این پروژه از ثابت‌های خود Eloquent (`Model::CREATED_AT`) استفاده می‌شود.
- **`DELETED_AT` روی اینترفیس مشکلی ندارد** اگر مدل از `SoftDeletes` استفاده کند، چون ثابت trait بر
  ثابت ارث‌بری‌شده از اینترفیس اولویت دارد (تست‌شده در همین ریپو).

## ۶. تله‌هایی که از پک یاد گرفتیم و در آن نمی‌افتیم
